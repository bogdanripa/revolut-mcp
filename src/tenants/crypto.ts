// Authenticated encryption for secrets at rest — the Revolut access and refresh
// tokens held on behalf of each business. AES-256-GCM; the key is
// REVOLUT_SESSION_KEY, 32 bytes as 64 hex chars.
...
export function keyFromHex(hex: string | undefined): Buffer {
  ...
  if (trimmed.length !== 64 || key.length !== 32) {
    throw new Error('REVOLUT_SESSION_KEY must be 32 bytes as 64 hex characters.');
  }
  return key;
}
